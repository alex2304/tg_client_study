...
BroadcastInner::BroadcastInner(SettingsWidget *parent) : TWidget(parent)
, _self(App::self())
...
, _photoOver(false)
, a_photoOver(0)
, _a_photo(animation(this, &BroadcastInner::step_photo))
...
void BroadcastInner::peerUpdated(PeerData *data) {
	if (self() && data == self()) {
...
void BroadcastInner::step_photo(float64 ms, bool timer) {
...
void BroadcastInner::updateOnlineDisplay() {
}

void BroadcastInner::updateConnectionType() {
	QString connection;
...
void BroadcastInner::passcodeChanged() {
	resizeEvent(0);
...
void BroadcastInner::gotPassword(const MTPaccount_Password &result) {
...
void BroadcastInner::offPasswordDone(const MTPBool &result) {
	onReloadPassword();
}

bool BroadcastInner::offPasswordFail(const RPCError &error) {
...
void BroadcastInner::usernameChanged() {
...
void BroadcastInner::saveError(const QString &str) {
	_errorText = str;
	resizeEvent(0);
	update();
}

void BroadcastInner::supportGot(const MTPhelp_Support &support) {
	if (!App::main()) return;
...
void BroadcastInner::onUpdatePhotoCancel() {
	if (self()) {
...
void BroadcastInner::onUpdatePhoto() {
	if (!self()) {
		return;
	}
...
void BroadcastInner::onShowSessions() {
	SessionsBox *box = new SessionsBox();
	Ui::showLayer(box);
}

void BroadcastInner::onAskQuestion() {
	if (!App::self()) return;
...
void BroadcastInner::onAskQuestionSure() {
	if (_supportGetRequest) return;
	_supportGetRequest = MTP::send(MTPhelp_GetSupport(), rpcDone(&BroadcastInner::supportGot));
}

void BroadcastInner::onTelegramFAQ() {
	QDesktopServices::openUrl(telegramFaqLink());
}

void BroadcastInner::chooseCustomLang() {
    QString file;
    QByteArray arr;
...
void BroadcastInner::onChangeLanguage() {
...
void BroadcastInner::onSaveTestLang() {
...
void BroadcastInner::onUpdateLocalStorage() {
	resizeEvent(0);
	updateSize(width());
	update();
}
...
#ifndef TDESKTOP_DISABLE_AUTOUPDATE
void BroadcastInner::onAutoUpdate() {
...
void BroadcastInner::onCheckNow() {
...
void BroadcastInner::onRestartNow() {
#ifndef TDESKTOP_DISABLE_AUTOUPDATE
	checkReadyUpdate();
...
void BroadcastInner::onPasswordOff() {
...
//		int32 flags = MTPDaccount_passwordInputSettings::flag_new_salt | MTPDaccount_passwordInputSettings::flag_new_password_hash | MTPDaccount_passwordInputSettings::flag_hint | MTPDaccount_passwordInputSettings::flag_email;
		...
		MTP::send(MTPaccount_UpdatePasswordSettings(MTP_bytes(QByteArray()), settings), rpcDone(&BroadcastInner::offPasswordDone), rpcFail(&BroadcastInner::offPasswordFail));
	} else {
...
void BroadcastInner::onReloadPassword(Qt::ApplicationState state) {
	if (state == Qt::ApplicationActive) {
		MTP::send(MTPaccount_GetPassword(), rpcDone(&BroadcastInner::gotPassword));
	}
}

void BroadcastInner::onAutoLock() {
...
#ifndef TDESKTOP_DISABLE_NETWORK_PROXY
void BroadcastInner::onConnectionType() {
...
void BroadcastInner::onWorkmodeWindow() {
...
void BroadcastInner::onAutoStart() {
	_startMinimized.setDisabled(!_autoStart.checked());
...
void BroadcastInner::onScaleChange() {
...
void BroadcastInner::setScale(DBIScale newScale) {
...
void BroadcastInner::onSoundNotify() {
	cSetSoundNotify(_soundNotify.checked());
	Local::writeUserSettings();
}

void BroadcastInner::onIncludeMuted() {
	cSetIncludeMuted(_includeMuted.checked());
	Notify::unreadCounterUpdated();
	Local::writeUserSettings();
}

void BroadcastInner::onWindowsNotifications() {
...
void BroadcastInner::onDesktopNotify() {
...
void BroadcastInner::enableDisplayNotify(bool enable)
{
	_desktopNotify.setChecked(enable);
}

void BroadcastInner::onSenderName() {
	_messagePreview.setDisabled(!_senderName.checked());
...
void BroadcastInner::onMessagePreview() {
	if (_messagePreview.checked()) {
		cSetNotifyView(dbinvShowPreview);
	} else if (_senderName.checked()) {
		cSetNotifyView(dbinvShowName);
...
void BroadcastInner::onReplaceEmojis() {
...
void BroadcastInner::onViewEmojis() {
	Ui::showLayer(new EmojiBox());
}

void BroadcastInner::onStickers() {
	Ui::showLayer(new StickersBox());
}

void BroadcastInner::onEnterSend() {
...
void BroadcastInner::onCtrlEnterSend() {
...
void BroadcastInner::onBackFromGallery() {
	BackgroundBox *box = new BackgroundBox();
	Ui::showLayer(box);
}

void BroadcastInner::onBackFromFile() {
	QStringList imgExtensions(cImgExtensions());
	QString filter(qsl("Image files (*") + imgExtensions.join(qsl(" *")) + qsl(");;All files (*.*)"));
...
void BroadcastInner::updateChatBackground() {
...
void BroadcastInner::needBackgroundUpdate(bool tile) {
	_needBackgroundUpdate = true;
	_tileBackground.setChecked(tile);
	updateChatBackground();
}

void BroadcastInner::onTileBackground() {
...
void BroadcastInner::onAdaptiveForWide() {
...
void BroadcastInner::onDontAskDownloadPath() {
...
void BroadcastInner::onDownloadPathClear() {
...
void BroadcastInner::onDownloadPathClearSure() {
	Ui::hideLayer();
...
void BroadcastInner::onLocalStorageClear() {
...
void BroadcastInner::onTempDirCleared(int task) {
...
void BroadcastInner::onTempDirClearFailed(int task) {
	if (task & Local::ClearManagerDownloads) {
...
void BroadcastInner::onAutoDownload() {
	Ui::showLayer(new AutoDownloadBox());
}

#ifndef TDESKTOP_DISABLE_AUTOUPDATE
void BroadcastInner::setUpdatingState(UpdatingState state, bool force) {
...
void BroadcastInner::setDownloadProgress(qint64 ready, qint64 total) {
...
void BroadcastInner::onUpdateChecking() {
	setUpdatingState(UpdatingCheck);
}

void BroadcastInner::onUpdateLatest() {
	setUpdatingState(UpdatingLatest);
}

void BroadcastInner::onUpdateDownloading(qint64 ready, qint64 total) {
	setUpdatingState(UpdatingDownload);
	setDownloadProgress(ready, total);
}

void BroadcastInner::onUpdateReady() {
	setUpdatingState(UpdatingReady);
}

void BroadcastInner::onUpdateFailed() {
	setUpdatingState(UpdatingFail);
}
#endif

void BroadcastInner::onPhotoUpdateStart() {
	showAll();
	update();
}

void BroadcastInner::onPhotoUpdateFail(PeerId peer) {
...
void BroadcastInner::onPhotoUpdateDone(PeerId peer) {
	if (!self() || self()->id != peer) return;
	showAll();
	update();
}
